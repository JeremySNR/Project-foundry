...
- One run targets one repo. The parent/child run model + epic rollup (#35) has
  ...
  roles regardless of risk (additive, lock-step Rego). A **starter policy
  library** has landed too (`policy/library/`, `foundry-policy` CLI, issue #31):
  vetted `baseline`/`soc2`/`change-management` presets a buyer copies into
  `foundry.yaml` — built only from existing knobs, copy-to-adopt (never
  auto-applied), so it adds no gate mechanism. Per-*path* (sub-tree)
  approval rules, N-of-M *count* approval matrices, and **live user-loadable** OPA
  bundles remain open on the programmable-policy epic (#31).

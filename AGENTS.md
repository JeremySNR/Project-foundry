...
- Single-tenant DB (no `org_id` / row-level isolation yet). API auth accepts
  ...
  unchanged. Still no SCIM, and the dashboard's browser-side OIDC login flow is
  not built (this covers bearer auth for the API the dashboard JS calls). Coarse

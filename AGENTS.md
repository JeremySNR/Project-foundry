...
- One run targets one repo. The parent/child run model + epic rollup (#35) has
  ...
  (`build_epic_evidence_pack`, `GET /runs/{id}/epic/evidence`), but the
  *producer* — decomposing an epic ticket into independently-gated child plans —
  plus path-scoped policy are still open.

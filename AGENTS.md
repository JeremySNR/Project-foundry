...
- One run targets one repo. The parent/child run model + epic rollup (#35) has
  ...
  default off, wired through `InlineDriver`); what remains is LLM-assisted
  decomposition (the producer is deterministic). Path-scoped policy has a first slice: per-repo forbidden-path

...
- Single-tenant DB (no `org_id` / row-level isolation yet). API auth now accepts
  OIDC JWTs as well as the static bearer token (#34), but SSO is auth-only:
  approver roles are still committed config (no IdP-group → role mapping, no
  SCIM). Coarse per-client rate limiting fronts the webhook + API surfaces.

...
| `policy/library/` | Starter policy library (issue #31): committed, tested preset configs (`baseline`, `soc2`, `change-management`) a buyer can copy into `foundry.yaml` instead of starting from a blank file. Built **only** from existing, already-gated knobs (`repo_confidence_threshold`, `forbidden_globs`, `repo_required_roles`, retry/budget caps) — no new policy mechanism, no `engine.py`/`foundry.rego` change, no audit write, so no lock-step concern. **Copy-to-adopt, never auto-applied** (loading the library can't change a running deployment's gate, so it can't weaken one — invariant #1); each preset is itself strict-or-stricter than the built-in defaults (a test asserts none drops a default forbidden glob). `library/__init__.py` is the pure, offline loader (`list_presets` / `load_preset_yaml` / `load_preset_settings` via `importlib.resources` + the real `Settings` validator). `policy/cli.py` is the `foundry-policy` console entry (`presets` / `show <name>` copy-to-adopt / `explain <name>` effective-knobs — decision-support only) |
...
- Single-tenant DB (no `org_id` / row-level isolation yet). API auth accepts
  ...
  human gate. Still no SCIM and no `org_id`/row-level isolation. Coarse
  per-client rate limiting fronts the webhook + API surfaces.
...
- One run targets one repo. The parent/child run model + epic rollup (#35) has
  ...
  *distinct* human approvers before it leaves `WAITING_APPROVAL` for `APPROVED`
  ...
  copy-to-adopt (never auto-applied), so it adds no gate mechanism. **Per-*path*
  ...
  (invariant #2 doesn't apply); default empty = unchanged. **Live user-loadable**
  OPA bundles remain the one open headline AC on the programmable-policy epic
  (#31).

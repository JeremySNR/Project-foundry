...
| `api/` | FastAPI: signed webhooks (Linear/GitHub/Jira/GitLab), REST approvals, run timeline, `GET /metrics/delivery`, `GET /metrics/agents` (per-provider scorecards), and a zero-build HTML dashboard at `/dashboard`. Everything token-gated, **fail-closed** (no `FOUNDRY_API_TOKEN` = endpoints disabled) |

...
| `db/models.py` | SQLAlchemy: runs, artifacts, audit events, policy decisions, agent jobs, repo catalog, run outcomes, webhook deliveries (dedup). **Single-tenant** — no org/tenant columns. Artifact `content_json` uses the `EncryptedText` column type (`db/encryption.py`): transparently encrypted at rest when `FOUNDRY_ARTIFACT_ENCRYPTION_KEY` is set (Fernet; comma-separated keys rotate via `MultiFernet`), plaintext otherwise. The `content_hash` is over **plaintext** canonical JSON, so encryption never disturbs integrity verification or hash dedup; the SQL type stays `TEXT`, so there is no migration. `cryptography` is the optional `[crypto]` extra, imported lazily so the no-key path never needs it |
...
- Single-tenant DB; approvers are static config (no SSO/SCIM). Artifact payloads
  can be encrypted at rest (`FOUNDRY_ARTIFACT_ENCRYPTION_KEY`, opt-in); `org_id` +
  row-level isolation and OIDC/SCIM are still open (#34).

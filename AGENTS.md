...
- One run targets one repo. The parent/child run model + epic rollup (#35) has
  ...
  (`engines/decomposition.py`, `orchestrator.intake_epic`); what remains is
  wiring epic detection into a trigger/webhook auto-path (today `intake_epic`
  is an explicit entrypoint) and LLM-assisted decomposition (the producer is
  deterministic). Path-scoped policy has a first slice: per-repo forbidden-path
